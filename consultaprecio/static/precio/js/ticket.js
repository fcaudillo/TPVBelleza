...
function getProductoByCodigoInterno (codigo) {
   inline = true;
   if (inline)
...
function sendTicketPrinter (ticket_json,result) {
   ...
   ticket_promise  =  new Promise(function(resolve, reject) {
	            var jsonData = JSON.stringify(ticket_impresora);
		    $.ajax({
				type: 'POST', // Use POST with X-HTTP-Method-Override or a straight PUT if appropriate.
				dataType: 'json', // Set datatype - affects Accept header
				url: url_printer, // A valid URL
...
function createTicketPrint(ticket,result) {
  var re = new RegExp('&quot;', 'g');
  var adicional_encabezado = adicional.replace(re,'"').split('|');
  var adicional_pie = ticket_pie.replace(re,'"').split('|');
  var productos = []
  ...
  var ticket_print = {
     'encabezado': {
                     'giro': cliente_giro,
                     'negocio': cliente_nombre,
                     'fecha': result.fecha,
                     'adicional': adicional_encabezado
                   },
     'productos': productos,
     'pie': {
             'numero_ticket': result.folio,
             'adicional': adicional_pie
     } 
...
$(document).ready(function() {
	...
	$( "#btnSearch" ).click(function() {
			...
			var codigo = $( "#codigobarras" ).val()
                        getProductoByCodigoInterno(codigo).subscribe(function (result) {
