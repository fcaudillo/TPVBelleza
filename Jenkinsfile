 node {
   ...
   stage('Deploy a produccion') {
       ...
       sh "docker run --network=main_default -e USUARIO_DB=${USUARIO_DB} -e PASSWORD_DB=${PASSWORD_DB} -v /home/dockeradm/data/tpv-verde:/app/tlapape/data --rm fcaudillo/tpv-verde:lts ./manage.py migrate " 
